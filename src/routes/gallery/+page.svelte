<script lang="ts">
	...
	const filteredProjects = $derived.by(() => {
		return data.projects.filter(project => {
			// Service filter
			if (selectedService && !project.services?.some(s => s.slug.current === selectedService)) {
				return false;
			}
	...
	const getMediaCount = (project: any) => {
		...
		if (project.beforeAfterImages?.before?.length) count += project.beforeAfterImages.before.length;
		if (project.beforeAfterImages?.after?.length) count += project.beforeAfterImages.after.length;
		return count;
	};
</script>
		...
		<div class="container mx-auto px-4">
			<div class="text-center max-w-4xl mx-auto">
				...
				<p class="text-lg text-muted-foreground mb-8">
					Explore our complete collection of professional cleaning transformations. 
					See detailed before & after comparisons, project photos, and videos from our work across Vancouver, BC.
				</p>
				...
				<div class="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl mx-auto">
					...
					<div class="text-center">
						<div class="text-2xl font-bold text-primary">{data.stats.completed}</div>
						<div class="text-sm text-muted-foreground">Completed</div>
					</div>
					<div class="text-center">
						<div class="text-2xl font-bold text-primary">{data.stats.featured}</div>
		...
		<div class="container mx-auto px-4">
			{#if filteredProjects.length === 0}
				<div class="text-center py-16">
					<p class="text-lg text-muted-foreground mb-4">No projects found matching your filters.</p>
					<Button onclick={() => { selectedService = ''; showFeatured = false; }}>Reset Filters</Button>
				</div>
			{:else}
				<div class="space-y-16">
					{#each visibleProjects as project, index}
						<article class="bg-card rounded-xl border shadow-sm overflow-hidden">
							<!-- Project Header -->
							<div class="bg-gradient-to-r from-primary/5 to-secondary/5 p-6 border-b">
									...
									<div>
										<div class="flex items-center gap-3 mb-2">
											...
											<span class={cn(
												...
												project.status === 'in-progress' && "bg-yellow-500/20 text-yellow-700 dark:text-yellow-300"
											)}>
							...
							{#if project.beforeAfterImages.before?.[0] && project.beforeAfterImages.after?.[0]}
								<div class="mb-6">
									<BeforeAfterComparison 
										beforeImage={project.beforeAfterImages.before[0].url}
										afterImage={project.beforeAfterImages.after[0].url}
							...
							{#if (project.beforeAfterImages.before && project.beforeAfterImages.before.length > 1) || (project.beforeAfterImages.after && project.beforeAfterImages.after.length > 1)}
								<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
												...
												{#if project.beforeAfterImages.before && project.beforeAfterImages.before.length > 1}
													<div>
														<h4 class="font-medium mb-3 text-muted-foreground">Additional Before Images</h4>
														<div class="grid grid-cols-2 gap-3">
															{#each project.beforeAfterImages.before.slice(1) as image}
																<div class="relative rounded-lg overflow-hidden aspect-[4/3]">
																	<img 
																		src={image.url}
																		alt={image.alt || `Before - ${project.title}`}
																		class="w-full h-full object-cover"
																		loading="lazy"
																	/>
																	<div class="absolute bottom-2 left-2 bg-red-500/90 text-white text-xs px-2 py-1 rounded">
																		BEFORE
																	</div>
																</div>
															{/each}
												...
												{#if project.beforeAfterImages.after && project.beforeAfterImages.after.length > 1}
													<div>
														<h4 class="font-medium mb-3 text-muted-foreground">Additional After Images</h4>
														<div class="grid grid-cols-2 gap-3">
															{#each project.beforeAfterImages.after.slice(1) as image}
																<div class="relative rounded-lg overflow-hidden aspect-[4/3]">
																	<img 
																		src={image.url}
																		alt={image.alt || `After - ${project.title}`}
																		class="w-full h-full object-cover"
																		loading="lazy"
																	/>
																	<div class="absolute bottom-2 left-2 bg-green-500/90 text-white text-xs px-2 py-1 rounded">
																		AFTER
																	</div>
																</div>
															{/each}
								...
								{#if project.galleryImages && project.galleryImages.length > 0}
									<div class="mb-8">
										<h3 class="text-lg font-semibold mb-4">Project Gallery ({project.galleryImages.length} images)</h3>
										<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
											{#each project.galleryImages as image}
												<div class="rounded-lg overflow-hidden aspect-square group hover:shadow-lg transition-all duration-300">
													<img 
														src={image.url}
	...
	<section class="py-16 bg-gradient-to-r from-primary to-primary/80 text-primary-foreground">
		<div class="container mx-auto px-4 text-center">
			...
			<p class="text-lg mb-8 opacity-90 max-w-2xl mx-auto">
				Inspired by these transformations? Get your own professional cleaning service and 
				join our gallery of satisfied customers across Vancouver.
			</p>
			<div class="flex flex-col sm:flex-row gap-4 justify-center">
				<Button variant="secondary" size="lg" class="bg-white text-primary hover:bg-white/90">
